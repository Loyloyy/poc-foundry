...
def critic_adequacy_prompt(criterion: str, test_src: str, interface: str) -> str:
    return (
        ...
        "Beyond a constant, a shortcut is anything that mimics the output's surface WITHOUT doing the "
        "task's real work: echoing the matched document/data verbatim, a keyword/title lookup, or a small "
        "hard-coded table. Mark INADEQUATE if such a cheap, plausible shortcut would satisfy every "
        "assertion (e.g. a test that only checks the reply contains a word from the matched document is "
        "passed by a stub that just ECHOES that document). Trustworthy evidence requires passing to demand "
        "GENERALISATION a shortcut cannot fake — most reliably a positive input phrased DIFFERENTLY from "
        "the data (synonyms/paraphrase, not the data's own words) that still yields the correct result. "
        "You still do NOT need the test to PROVE which mechanism ran (pgvector, embeddings, …) — that is "
        "out of scope; judge only whether a cheap shortcut could fake the observed behaviour. Be lenient "
        "on form, strict on substance. Give a one-sentence `reason`; if inadequate, add a concrete "
        "`suggestion` (e.g. add a paraphrased-query case the matched document must still satisfy)."
    )
